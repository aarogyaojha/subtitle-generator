...
def test_distinct_speaker_count_computation(mock_run_pipeline, capsys):
    ...
    and not overcounted by multiple turns from the same speaker, and both
    native and English output paths are printed.
    """
    mock_run_pipeline.return_value = {
        "hardware_tier": "colab",
        "cue_count": 4,
        ...
        "srt_path_en": "output/test.en.srt",
        "vtt_path_en": "output/test.en.vtt",
    ...
    assert "Hardware tier: colab" in captured.out
    ...
    assert "Native SRT output: output/test.srt" in captured.out
    assert "Native VTT output: output/test.vtt" in captured.out
    assert "English SRT output: output/test.en.srt" in captured.out
    assert "English VTT output: output/test.en.vtt" in captured.out
...
@patch("src.cli.run_pipeline")
def test_cli_reports_translation_skipped_when_source_is_english(mock_run_pipeline, capsys):
    """Verify CLI reports a clear note when translation is skipped for English audio."""
    mock_run_pipeline.return_value = {
        "hardware_tier": "colab",
        "cue_count": 2,
        "cue_count_en": 0,
        "speaker_turns": [{"speaker_id": "SPEAKER_00"}],
        "srt_path": "output/english.srt",
        "vtt_path": "output/english.vtt",
        "srt_path_en": None,
        "vtt_path_en": None,
        "translation_skipped": True,
        "translation_skip_reason": "Source language is already English ('en')",
    }

    exit_code = main(["english.wav", "-l", "en"])
    assert exit_code == 0

    captured = capsys.readouterr()
    assert "Native SRT output: output/english.srt" in captured.out
    assert "English translation: Skipped (source audio is already English)" in captured.out
...
def test_cli_real_end_to_end_integration(tmp_path, capsys):
    ...
    assert "Native SRT output:" in captured.out
    assert "English SRT output:" in captured.out

    srt_file = out_dir / f"{NEPALI_AUDIO_FIXTURE.stem}.srt"
    vtt_file = out_dir / f"{NEPALI_AUDIO_FIXTURE.stem}.vtt"
    srt_en_file = out_dir / f"{NEPALI_AUDIO_FIXTURE.stem}.en.srt"
    vtt_en_file = out_dir / f"{NEPALI_AUDIO_FIXTURE.stem}.en.vtt"

    assert srt_file.exists(), f"Expected SRT file {srt_file} to exist"
    assert vtt_file.exists(), f"Expected VTT file {vtt_file} to exist"
    assert srt_en_file.exists(), f"Expected English SRT file {srt_en_file} to exist"
    assert vtt_en_file.exists(), f"Expected English VTT file {vtt_en_file} to exist"
    assert srt_file.stat().st_size > 0
    assert vtt_file.stat().st_size > 0
    assert srt_en_file.stat().st_size > 0
    assert vtt_en_file.stat().st_size > 0
